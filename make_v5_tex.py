...
\subsection{Phase 1: Instruction Dataset Synthesis}
...
\subsection{Phase 1: Multimodal ECG Alignment Data}
For the initial vision-language tuning phase, we utilized the \texttt{PULSE-ECG/ECGBench} dataset, specifically focusing on the \texttt{ptb-test-report} subset. This dataset maps raw ECG images (represented as high-resolution PIL objects) to rigorous clinical reports. These reports are not merely top-level classifications; they include granular morphological analyses, such as precise PR interval durations, QRS complex morphologies, and ST-segment deviations.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{dataset_dist.pdf}
    \caption{Distribution of pathologies within the initial curated Cardio-Sahayak Phase 1 instruction dataset. The dataset over-samples conditions relevant to the Indian demographic, including genetic cardiomyopathies (HCM/DCM) and early-onset STEMI presentations.}
    \label{fig:dataset_dist_v1}
\end{figure}

\subsection{Phase 2: Expanded Multimodal Dataset Acquisition (V2 Dataset)}
To further robustly generalize the foundation model's grasp of the nuances in South Asian healthcare environments, we engineered a highly specialized \textit{V2 Dataset} comprising 166 complex records, explicitly addressing the data drought in Indian cardiology.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{v5_dataset_dist.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{v5_phenotype_shift.pdf}
...
\subsection{Phase 1 Training Dynamics}
The initial training process was executed in two distinct stages using the Hugging Face \texttt{TRL} and \texttt{Transformers} libraries:

\textbf{Stage 1: Text Supervised Fine-Tuning (SFT)}
\begin{itemize}
...
\textbf{Stage 2: Multimodal VLM Tuning}
\begin{itemize}
...
\begin{figure}[H]
    ...
    \caption{Simulated training convergence for the Phase 1 Text Supervised Fine-Tuning (SFT) and Multimodal Vision-Language Model (VLM) runs. Both phases exhibited stable exponential decay in cross-entropy loss.}
    \label{fig:training_loss_v1}
\end{figure}

\subsection{Phase 2 Training Dynamics and Adapter Resumption}
To integrate the V2 Dataset without losing the deep reasoning capabilities learned in Phase 1, we resumed training directly from the saved Phase 1 LoRA adapters (\texttt{tp53/cardio-sahayak}). We utilized an adjusted learning rate of $1\times 10^{-4}$ across 3 additional epochs. This careful resumption allowed the model to internalize the complex JSON EMR structuring and demographic shifts without overwriting its foundational medical knowledge. The final refined adapters were successfully saved into a \texttt{v2\_weights} subfolder, maintaining repository consolidation.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{v5_training_loss.pdf}
...
\subsection{Architectural Patching for Gemma3 Compatibility}
During the GGUF conversion, we encountered challenges with the base \texttt{MedGemma-27B} model utilizing a novel \texttt{Gemma3} VLM architecture, where core LLM layers are nested under a \texttt{language\_model} attribute. Standard \texttt{llama.cpp} tools do not natively support this nested BPE pre-tokenizer structure. 

To overcome this, we implemented a sophisticated runtime patch during our pipeline:
\begin{enumerate}
    \item We structurally unwrapped the nested text model and reformatted its \texttt{config.json} to mimic a standard \texttt{Gemma2ForCausalLM} architecture, ensuring layer mapping compatibility.
    \item We executed a live runtime replacement against the \texttt{llama.cpp} source code to bypass the \texttt{NotImplementedError}, forcing the conversion script to utilize proven "gemma" pre-tokenization logic.
\end{enumerate}
These precise engineering interventions were critical in achieving the final, edge-ready V2 GGUF artifacts.
...
\end{document}
"""
with open("out/v5_cardio-sahayak_preprint.tex", "w") as f:
    f.write(content)
