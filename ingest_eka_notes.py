...
def format_eka_dataset():
    ...
    try:
        dataset = load_dataset("ekacare/clinical_note_generation_dataset", split="train", token=hf_token)
    ...
    for idx, item in enumerate(dataset):
        ...
        transcript = item.get('dialogue') or item.get('transcript') or item.get('input', '')
        note = item.get('clinical_note') or item.get('output') or item.get('summary', '')
        
        if not transcript or not note:
            continue
            
        instruction = f"Analyze the following medical consultation transcript and generate a structured clinical note suitable for an Indian healthcare setting. Pay attention to specific local terminologies or conditions if present.\n\nTranscript:\n{transcript}"
